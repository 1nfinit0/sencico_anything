...
def gran_circulo(xa,ya,xb,yb):
     a = (ya,xa)
     b = (yb,xb)
     distancia = great_circle(a, b).meters
     return distancia

def haversine(xa,ya,xb,yb):
    RADIO_TERRESTRE = 6371251.46
    x_dist=math.radians(xa-xb)
    y_dist=math.radians(ya-yb)

    y1_rad=math.radians(ya)
    y2_rad=math.radians(yb)

    a = math.sin(y_dist/2)**2+math.sin(x_dist/2)**2*math.cos(y1_rad)*math.cos(y2_rad)
    c = 2*math.asin(math.sqrt(a))
    distancia = c * RADIO_TERRESTRE
    return distancia

def vincenty(xa,ya,xb,yb):
    distance = None
    ...
    L = math.radians(xb-xa)
    U1 = math.atan((1-f) * math.tan(math.radians(ya)))
    U2 = math.atan((1-f) * math.tan(math.radians(yb)))
    sinU1 = math.sin(U1)
    cosU1 = math.cos(U1)
    sinU2 = math.sin(U2)
    cosU2 = math.cos(U2)
...
pitagorica = pitagoras(456456.23,1279721.064,576628.34,1071740.33)
grancirculo = gran_circulo(-90.212452861859035,32.316272202663704,-88.952170968942525,30.438559624660321)
...
resultados = [ep, eg, eh, ev]

print("Error absoluto: ",mae(media,resultados))